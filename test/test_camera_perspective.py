...
def test_perspective_attributes():
    cam = fresnel.camera.Perspective(
        position=(0, 0, -10), look_at=(0, 0, 0), up=(0, 1, 0), height=0.25, focal_length=1.25, focus_distance=20, f_stop=1.5)
...
def test_render(scene_, generate=False):
    conftest.check_preview_render(scene_, "test_camera_perspective.test_render", generate=generate)


def test_focal_length(scene_, generate=False):
    scene_.camera.focal_length = 0.25

    conftest.check_preview_render(scene_, "test_camera_perspective.test_focal_length", generate=generate)


def test_height(scene_, generate=False):
    scene_.camera.height = .125

    conftest.check_preview_render(scene_, "test_camera_perspective.test_height", generate=generate)


def test_pathtrace(scene_, generate=False):
    conftest.check_pathtrace_render(scene_, "test_camera_perspective.test_pathtrace", generate=generate)


def test_f_stop(scene_, generate=False):
    scene_.camera.f_stop = 0.2

    conftest.check_pathtrace_render(scene_, "test_camera_perspective.test_f_stop", generate=generate)


def test_focal_distance(scene_, generate=False):
    scene_.camera.f_stop = 0.2
    scene_.camera.focus_distance = 6

    conftest.check_pathtrace_render(scene_, "test_camera_perspective.test_focal_distance", generate=generate)
...
if __name__ == '__main__':
    ...
    for method in [test_render, test_focal_length, test_height, test_pathtrace, test_f_stop, test_focal_distance,]:
        s = scene(device)
