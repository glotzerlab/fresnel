...
def test_lightlist(device):
    ...
    assert(len(l) == 3)
    l.append(fresnel.light.Light(direction=(5,1,2), color=(16, 32, 64), theta=.75))
    assert(len(l) == 4)
    ...
    scene.lights = [fresnel.light.Light(direction=(1,2,3)), fresnel.light.Light(direction=(10,20,30), color=(1,0,0), theta=1.125)];
    assert(l[0].direction == (1,2,3))
