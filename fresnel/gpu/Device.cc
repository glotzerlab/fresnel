...
/*! Construct a new optix::Context with default options.
    ...
    OptiX programs are loaded from PTX files, built from the .cu source files. These PTX files are
   stored in the python library directory. The Device instance tracks this directory for other
...
static std::string _formatOptiXDeviceList(const std::vector<int>& devices)
    ...
    for (const int& i : devices)
        ...
        int clock_rate = 0;
        RTsize total_mem = 0;
        optix::ContextObj::getDeviceAttribute(i,
...
/*! \param filename Name of the file to load the program from
    ...
    A file `filename` is assumed to exist in the ptx root dir. If this program has already been
   loaded, a shared pointer to it is returned. If it has not yet been loaded, it will be loaded
   first.
*/
...
/*! \param filename Name of the file to load the program from
    ...
    If the entry point for this program already exists in the cache, return it. If not, load the
