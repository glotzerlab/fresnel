...
/*! Geometry tracks OptiX GeometryInstance objects that belong to a given Scene.
    ...
    For derived classes, the bool value m_valid is true when the Geometry is added to the scene.

    Note: Do not confuse Geometry with optix::Geometry in the following:
    The full configuration of a Geometry is split between the base class generic methods and the derived class
    specifics. First, the derived class should allocate memory, buffers, set intersection programs, and initialize
