...
static __device__ float getShadingDistance(vec3<float> n_hit, vec3<float> p_hit, vec3<float> ray_dir_local, vec3<float> r_hit, int n_planes)
    {
    float min_d = FLT_MAX;
...
RT_PROGRAM void intersect(int item)
    ...
    if (rtPotentialIntersection(t0))
        ...
        n_hit = t0_n_local;
        p_hit = t0_p_local;
        shading_color = lerp(convex_polyhedron_color_by_face, RGB<float>(convex_polyhedron_color[item]), RGB<float>(convex_polyhedron_plane_color[t0_plane_hit]));
        shading_distance = getShadingDistance(n_hit, p_hit, ray_dir_local, ray_org_local + t0 * ray_dir_local, n_planes);
        ...
        n_hit = t1_n_local;
        p_hit = t1_p_local;
        shading_color = lerp(convex_polyhedron_color_by_face, RGB<float>(convex_polyhedron_color[item]), RGB<float>(convex_polyhedron_plane_color[t1_plane_hit]));
