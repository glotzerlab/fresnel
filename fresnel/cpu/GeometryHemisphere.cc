...
#include "common/IntersectHemisphere.h"
...
GeometryHemisphere::GeometryHemisphere(std::shared_ptr<Scene> scene, unsigned int N)
    ...
    m_director = std::shared_ptr< Array< vec3<float> > >(new Array< vec3<float> >(N));
    ...
    m_device->checkError();
    rtcSetGeometryBoundsFunction(m_geometry, &GeometryHemisphere::bounds, NULL);
    m_device->checkError();
    rtcSetGeometryIntersectFunction(m_geometry, &GeometryHemisphere::intersect);
    m_device->checkError();
...
/*! Compute the bounding box of a given primitive
    ...
    \param ptr Pointer to a GeometryHemisphere instance
    \param item Index of the primitive to compute the bounding box of
    \param bounds_o Output bounding box
*/
void GeometryHemisphere::bounds(const struct RTCBoundsFunctionArguments *args)
    {
    GeometryHemisphere *geom = (GeometryHemisphere*)args->geometryUserPtr;
...
/*! Compute the intersection of a ray with the given primitive
    ...
    \param item Index of the primitive to compute the bounding box of
*/
void GeometryHemisphere::intersect(const struct RTCIntersectFunctionNArguments *args)
    {
    GeometryHemisphere *geom = (GeometryHemisphere*)args->geometryUserPtr;
    const vec3<float> position = geom->m_position->get(args->primID);
    const quat<float> orientation = geom->m_orientation->get(args->primID);
    const vec3<float> director = geom->m_director->get(args->primID);
    ...
    float t=0, d=0;
    vec3<float> N;
    bool hit = intersect_ray_hemisphere(t,
                                    d,
                                    N,
