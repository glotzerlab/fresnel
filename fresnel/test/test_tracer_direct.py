...
def test_render(scene_hex_sphere_, generate=False):
    ...
    tracer = fresnel.tracer.Preview(
        device=scene_hex_sphere_.device, w=100, h=100, anti_alias=False
    )
    buf = tracer.output[:]
    ...
    if generate:
        ...
        conftest.assert_image_approx_equal(
            buf_proxy[:],
            dir_path / "reference" / "test_tracer_direct.test_render.png",
        )
...
def test_render_aa(scene_hex_sphere_, generate=False):
    ...
    tracer = fresnel.tracer.Preview(
        device=scene_hex_sphere_.device, w=100, h=100, anti_alias=True
    )
    buf = tracer.output[:]
    assert buf.shape == (100, 100, 4)
