...
def scene(device):
    ...
    scene.camera = fresnel.camera.Perspective(
        position=(0, 4, -10), look_at=(0, 0, 0), up=(0, 1, 0), focus_distance=10
    )
...
def test_render(scene_, generate=False):
    ...
    conftest.check_preview_render(
        scene_, "test_camera_perspective.test_render", generate=generate
    )
...
def test_focal_length(scene_, generate=False):
    ...
    conftest.check_preview_render(
        scene_, "test_camera_perspective.test_focal_length", generate=generate
    )


def test_height(scene_, generate=False):
    """Test the height property."""
    scene_.camera.height = 0.125

    conftest.check_preview_render(
        scene_, "test_camera_perspective.test_height", generate=generate
    )
...
def test_pathtrace(scene_, generate=False):
    ...
    conftest.check_pathtrace_render(
        scene_, "test_camera_perspective.test_pathtrace", generate=generate
    )
...
def test_f_stop(scene_, generate=False):
    ...
    conftest.check_pathtrace_render(
        scene_, "test_camera_perspective.test_f_stop", generate=generate
    )
...
def test_focus_distance(scene_, generate=False):
    ...
    conftest.check_pathtrace_render(
        scene_, "test_camera_perspective.test_focal_distance", generate=generate
    )
