...
def _validate_or_generate_image(buf, name, generate, tolerance):
    ...
    if generate:
        ...
        assert_image_approx_equal(
            buf[:], dir_path / "reference" / f"{name}.png", tolerance
        )


def check_preview_render(
    scene, name, w=150, h=100, tolerance=2, anti_alias=False, generate=False
):
...
def check_pathtrace_render(
    scene, name, w=150, h=100, tolerance=2, samples=64 * 40, generate=False
):
