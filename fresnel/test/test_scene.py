...
def test_background_color(device_):
    ...
    scene.camera = fresnel.camera.Orthographic(
        position=(0, 0, 10), look_at=(0, 0, 0), up=(0, 1, 0), height=7
    )
    buf_proxy = fresnel.preview(scene, w=100, h=100, anti_alias=False)
    buf = buf_proxy[:]

    numpy.testing.assert_array_equal(
        buf[:, :, 3], numpy.ones(shape=(100, 100), dtype=buf.dtype) * 128
    )
    numpy.testing.assert_array_equal(
        buf[:, :, 0:3],
...
def test_camera(scene_hex_sphere_, generate=False):
    ...
    scene_hex_sphere_.camera = fresnel.camera.Orthographic(
...
def test_multiple_geometries(device_, generate=False):
    ...
    geom1 = fresnel.geometry.Sphere(
        scene,
        position=[[-4, 1, 0], [-4, -1, 0], [-2, 1, 0], [-2, -1, 0]],
        radius=1.0,
    )
    geom1.material = fresnel.material.Material(
        solid=1.0, color=fresnel.color.linear([0.42, 0.267, 1])
    )
    geom1.outline_width = 0.12

    geom2 = fresnel.geometry.Sphere(
        scene,
        position=[[4, 1, 0], [4, -1, 0], [2, 1, 0], [2, -1, 0]],
        radius=1.0,
    )
    ...
    if generate:
        PIL.Image.fromarray(buf_proxy[:], mode="RGBA").save(
            open("output/test_scene.test_multiple_geometries1.png", "wb"), "png"
        )
    else:
        conftest.assert_image_approx_equal(
            buf_proxy[:],
    ...
    if generate:
        PIL.Image.fromarray(buf_proxy[:], mode="RGBA").save(
            open("output/test_scene.test_multiple_geometries2.png", "wb"), "png"
        )
    else:
        conftest.assert_image_approx_equal(
            buf_proxy[:],
    ...
    if generate:
        PIL.Image.fromarray(buf_proxy[:], mode="RGBA").save(
            open("output/test_scene.test_multiple_geometries3.png", "wb"), "png"
        )
    else:
        conftest.assert_image_approx_equal(
            buf_proxy[:],
    ...
    if generate:
        PIL.Image.fromarray(buf_proxy[:], mode="RGBA").save(
            open("output/test_scene.test_multiple_geometries4.png", "wb"), "png"
        )
    else:
        conftest.assert_image_approx_equal(
            buf_proxy[:],
